        ...
        for compactness in tqdm([0.1, 1, 10], desc="Compactness"):
            ...
            for n_segment in tqdm(range(0, 800, 50), desc="N_Segments"):
                if n_segment == 0:
                    ...
                    df_metrics_mask_p = pd.DataFrame(metrics_mask_p)
                    df_metrics_mask_p.to_csv(
                        path_output / mask / 'SLIC' / f'split_{s_n}' / mode / f'{compactness}' / f"metrics_{mask}_slic_{mode}_percentage.csv")
                else:
                    ...
                    print(metrics_exp)
                    metrics_mask[
                        f'{n_segment}'] = metrics_exp

            df_metrics_mask = pd.DataFrame(metrics_mask)
            df_metrics_mask.to_csv(
                path_output / mask / 'SLIC' / f'split_{s_n}' / mode / f'{compactness}'/ f"metrics_{mask}_slic_{mode}.csv")
