...
class ANTs(AbstractMethod):
    def __init__(self, path: Path, ref_t: int = 270, domain: str = None, **config):
        super().__init__()
        self.domain = domain
    ...
    def apply(self, patient, **kwargs):
        ...
        fix_filename = MRImage.resolve_name(patient.id, self.reference_t)
        ...
        if len(mov_filenames):
            ...
            os.makedirs(self.path_reg / "images", exist_ok=True)
            os.makedirs(self.path_reg / "ants_transforms", exist_ok=True)
            ...
            fix_img_ants = ants.from_nibabel(
                nib.Nifti1Image(patient.get_image(self.reference_t).data,
                                **nifti_args))
            nib.save(ants.to_nibabel(fix_img_ants),
                     str(self.path_reg / "images" / fix_filename))

            print("\n")
            for mov_img_filename in tqdm(mov_filenames,
                                         desc="Registering images from "
                                              "Patient: " + str(patient.id)):
                # reading moving image

                id, t = MRImage.parse_name(mov_img_filename)
                nifti_args_mov = patient.images(t).nifti_props
                ...
                print(f"{mov_img_filename} to {fix_filename}...")
                ...
                nib.save(warped_mov_img,
                         str(self.path_reg / "images" / mov_img_filename))

                # saving transforms (forward and inverted) from the registration
                tfm_fwd_filename = f"{patient.id:05d}_{t}_{self.reference_t}.mat"
                tfm_inv_filename = f"{patient.id:05d}_{self.reference_t}_{t}.mat"

                ants.write_transform(tfm_fwd,
                                     str(self.path_reg / "ants_transforms" /
                                         tfm_fwd_filename))
        ...
        if self.domain == 'REG':
            new_patient = Patient(path=self.path_reg / "images",
