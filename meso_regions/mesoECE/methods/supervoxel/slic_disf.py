...
class SLIC_DISF(AbstractMethod):
    ...
    def apply(self, patient: Patient, **kwargs):
        ...
        try:
            ...
            # for each lable, run DISF to get the supervoxels save all in one mask
            rps = skimage.measure.regionprops(supervoxels_mask)
            for rp in rps:
                new_mask = np.zeros_like(supervoxels_mask)
                slice_bbox = tuple(
                ...
                nib.save(nib.Nifti1Image(new_mask, **nifti_args),
                         str(self.path_supervoxels / 'temp.nii.gz'))
                ...
                cmd = (f"{self.ift_path}/bin/iftDISF {path_img} "
                       f"{50} {4} {path_out_mask} {path_mask}")

                # print(f"New segment pushed with volume: {
                # define_masks_volume(div_mask)}")
            # supervoxels = np.unique(supervoxels_mask)
            # centroids = []
            # for supervoxel in supervoxels:
            #     supervoxel_mask = np.where(supervoxels_mask == supervoxel)
            #     centroid = np.mean(supervoxel_mask, axis=1)
